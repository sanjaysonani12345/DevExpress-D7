...
{$ELSE}
  TdxMemDataValueBuffer = TValueBuffer;
{$ENDIF}
...
begin
  case State of
    dsBrowse: if IsEmpty then ARecordBuffer := nil else ARecordBuffer := ActiveBuffer;
    dsEdit, dsInsert: ARecordBuffer := ActiveBuffer;
    dsCalcFields: ARecordBuffer := CalcBuffer;
  else
    ARecordBuffer := nil;
  end;
...
begin
  if (FBlobList <> nil) and (FCurRec >= 0) and (FCurRec < RecordCount) and (State in dsEditModes) then
    SetBlobData(ActiveBuffer, Field, GetBlobData(TdxMemDataValueBuffer(FBlobList[FCurRec]), Field.Offset))
  else
...
begin
  ...
  cxZeroMemory(Buffer, BlobFieldCount * SizeOf(TdxNativeInt));
end;
...
begin
  ...
  begin
    ptr := ShiftPointer(Buffer, I * SizeOf(TdxNativeInt));
    ptr := ReadPointer(ptr);
    FreeMem(ptr);
  end;
end;
